#ifndef _MODEL_MSGPACK_HPP_
#define _MODEL_MSGPACK_HPP_
...
namespace json
{
	...
	public:
		...
		document() = default;
		explicit document(element_type&& root) : _root(std::forward<element_type>(root)) { }

		std::string save(const int indent = -1,	const char indent_char = ' ',
		...
		const element_type& child(const typename element_type& element, const char* name) const
		{
			return element.at(name);
		}
		typename element_type& child(typename element_type& element, const char* name) const
		{
			return element[name];
		}

		template<typename T>
		void put(typename element_type& element, const T& v) const
		{
			element = v;
		}

		template<typename T>
		void get(const typename element_type& element, T& v) const
		{
			element.get_to(v);
		}
